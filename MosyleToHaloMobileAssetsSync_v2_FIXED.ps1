...
.DESCRIPTION
  Reads a Mosyle JSON export (array of objects), connects to Halo via OAuth2 client_credentials,
  and performs a reconciliation against a specified Customer (client_id):

  1) For each Mosyle device (matched by serial_number):
     - If no Halo asset exists with the same serial number -> CREATE
     - If exists -> UPDATE only when fields differ

     Mappings (per your requirements):
       - Serial Number  -> Halo key_field (and inventory_number, optional)
       - Manufacturer   -> "Apple" (manufacturer_name)
       - Model          -> Mosyle osversion (stored in key_field2)
       - Notes          -> Mosyle device_name
       - Site           -> Default site (first active site for client unless specified)
       - Asset Type     -> "Mobile Device" asset type (resolved by name unless specified)

  2) After processing Mosyle list:
     - For every Halo asset of type "Mobile Device" for that client,
       if its serial number is NOT present in Mosyle export -> set inactive = true

  IMPORTANT:
    Halo's schema indicates Device objects include an `inactive` boolean for activation state.
    Asset listing supports filtering by client_id and assettype_id.
    Sites can be listed and filtered by client_id.
    Halo API authentication is OAuth2 and access tokens are provided as Bearer tokens.

.NOTES
  Recommended save locations:
    - Script:  C:\Users\ScottHolzberger\OneDrive - ZaheZone\Scripts\MosyleToHaloMobileAssetsSync_v2.ps1
    - Halo API credential (DPAPI protected):  %APPDATA%\ZaheZone\Halo\halo_api_app.xml

  Credentials are stored using Export-Clixml (DPAPI per-user/per-machine) so only the same Windows user on the
  same machine can decrypt.
#>
...
function Save-HaloApiAppCredential {
    <#
      Stores Halo API ClientID/ClientSecret as a PSCredential (UserName=ClientID, Password=ClientSecret).
      DPAPI-protected per Windows user.
    #>
    [CmdletBinding()]
    param(
...
function Connect-HaloApi {
    ...
    try {
        ...
        throw "Halo token request failed: $($_.Exception.Message) (URI=$tokenUri). If hosted, set -HaloAuthServer per Halo API settings."
    }
...
# Halo lookups
# -----------------------------
function Get-HaloAssetTypeIdByName {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]$Session,
    ...
    if (-not $types -or $types.Count -eq 0) {
        throw "Could not find any asset types matching '$Name' via /api/assettype. Provide -AssetTypeId explicitly."
    }

    $exact = $types | Where-Object { $_.PSObject.Properties.Name -contains 'name' -and $_.name -ieq $Name } | Select-Object -First 1
    if ($exact) { return [int]$exact.id }

    return [int]($types | Select-Object -First 1).id
}
...
function Sync-MosyleJsonToHaloMobileAssets {
    ...
    if (-not $AssetTypeId) { $AssetTypeId = Get-HaloAssetTypeIdByName -Session $session -Name $AssetTypeName }
    if (-not $DefaultSiteId) { $DefaultSiteId = Get-HaloDefaultSiteId -Session $session -CustomerId $CustomerId }
...
<#
.EXAMPLE
  # One-time: store Halo API app credentials (Client ID + Client Secret)
  ...
  # Run a dry-run first
  . .\MosyleToHaloMobileAssetsSync_v2.ps1
